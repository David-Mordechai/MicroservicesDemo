...
{
    private readonly IAeroLogger<MinIoClientBuilder> _logger;
    private readonly MinIoConfiguration _configuration;


    public MinIoClientBuilder(IAeroLogger<MinIoClientBuilder> logger, MinIoConfiguration configuration)
    {
        _logger = logger;
    ...
    private async Task CreateMapsBucket(IBucketOperations bucketOperations, string bucketName)
    {
        ...
        {
            _logger.LogError("MapsBucket does not exists or creation failed!", e);
        }
    }
}